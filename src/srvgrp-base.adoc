...
* Initial handshaking between the RPMI client(application processor) and the 
RPMI service provider(platform microcontroller).

* Querying the message framework implementation version information.

* Probing the service group implementation status.

* Gathering allowed system information from the platform.

* Configurations from RPMI client before the system management and control tasks.
...
was processing when the error occured.
|===
...
Get the implementation version of the RPMI framework from the Platform
Microcontroller. Versioning is also done for the specification but it's
possible that for a single version of the specification to have multiple
...
returned by this service is a `32-bit` composite number containing both Major
and Minor numbers.
...
Get the RPMI implementation ID assigned to platform implementing the RPMI
...
[#table_base_implementation_ids]
[cols="1, 1", width=100%, align="center", options="header"]
|===
...
This service can also be used to provide platform specific ids or version through
`HW_ID` field.
...
Notifications within the service group remain optional.
...
|===
| Word	| Name 		| Type		| Description
| 0	| SERVICEGROUP_ID | uint32	| `16-bit` ID assigned to each service
					group.
|===
...
| 1	| SERVICE_GROUP_STATUS	| uint32 | Service group implementation 
	...
	0: Service group not implemented.
	1: Service group implemented.
|===
...
Microcontroller can use as a doorbell to the AP. The PuC to AP MSI can be used 
for both sending MSI or injecting wired interrupts. 

NOTE: If the MSI target address is IMSIC, then the AP will take MSI
...
|===
| Word  | Name          	| Type          | Description
| 0     | MSI_ADDRESS_LOW	| uint32	| Lower `32-bit` of the MSI address
| 1     | MSI_ADDRESS_HIGH	| uint32	| Upper `32-bit` of the MSI address
