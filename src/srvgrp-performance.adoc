...
* The doorbell details such as doorbell register address and set value can
...
* The doorbell register address is the physical address of the register.
  The doorbell set value is the value which must be written in the doorbell register
  to trigger the doorbell interrupt. The width of the doorbell write value must
  be equal to the doorbell register width.

  NOTE: The set value may also contains other set bits which must persist on every write to the doorbell register.
...
| DB_SET_VALUE
| uint32
| 32-bit doorbell set value for Performance Request fast-channel. +
If the doorbell register width is less than 32-bit, the lower bits in this field
equal to the doorbell register width must be used as set value.
